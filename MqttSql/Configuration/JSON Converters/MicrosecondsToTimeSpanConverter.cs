...
public sealed class MicrosecondsToTimeSpanConverter : JsonConverter<TimeSpan>
{
    public override TimeSpan Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        double microseconds = JsonSerializer.Deserialize<double>(ref reader, options);
        return TimeSpan.FromMicroseconds(microseconds);
    }

    public override void Write(Utf8JsonWriter writer, TimeSpan value, JsonSerializerOptions options)
    {
        double microseconds = value.TotalMicroseconds;
        JsonSerializer.Serialize(writer, microseconds);
    }
}
